...
public class ModArenaRoom extends PatchRoom {

	@Override
	/*
	public int minWidth() { return sizeCat.minDim+1; }
	public int minHeight() { return sizeCat.minDim+1; }
	*/

	//mod: generation Arena size
	public int minWidth() { return 31; }
	public int minHeight() { return 31; }
	...
	public void paint(Level level) {
		...
		int[] mask = {Terrain.CHASM, Terrain.EMPTY, Terrain.WALL, Terrain.WALL_DECO, Terrain.WATER};


		this.left = (int) (level.width() / 2) - (int)(minWidth() / 2);
		this.top = (int) (level.height() / 2) - (int)(minHeight() / 2);
		this.right = this.left + (int) minWidth();
		this.bottom = this.top + (int) minHeight();


		Painter.fillDiamond( level, this, Terrain.WALL, mask);

		Painter.fillDiamond( level, this , 1 , Terrain.EMPTY, mask );
